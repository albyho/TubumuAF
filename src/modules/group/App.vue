...
<el-container v-loading.fullscreen.lock="isLoading">
  ...
  <el-main class="main">
    ...
    <el-row>
      <el-tree :data="treeData" :props="treeDefaultProps" :empty-text="mainTableEmptyText" node-key="id" ref="tree" :filter-node-method="filterNode" :render-content="renderContent" :default-expand-all="true" @node-drag-start="handleDragStart" @node-drag-enter="handleDragEnter" @node-drag-leave="handleDragLeave" @node-drag-over="handleDragOver" @node-drag-end="handleDragEnd" @node-drop="handleDrop" draggable :allow-drop="allowDrop" :allow-drag="allowDrag" />
    </el-row>
        ...
        <el-tabs v-model="activeTabName" type="card">
          <el-tab-pane label="基本信息" name="first">
            <el-form-item label="所属分组">
              <el-cascader :options="editParentTreeData" :props="editParentTreeDefaultProps" clearable change-on-select v-model="mainForm.parentIDPath" />
            </el-form-item>
            <el-form-item label="分组名称" prop="name">
              <el-input v-model.trim="mainForm.name" auto-complete="off" placeholder="请输入分组名称" ref="name" />
            </el-form-item>
    ...
    <el-dialog :visible.sync="moveFormDialogVisible" @submit.native.prevent :close-on-click-modal="false" width="600px">
      ...
      <el-form ref="moveForm" :model="moveForm" :rules="moveFormRules" label-position="right" label-width="100px">
        <el-form-item label="目标节点" prop="targetIDPath">
          <el-cascader :options="editParentTreeData" :props="editParentTreeDefaultProps" clearable change-on-select v-model="moveForm.targetIDPath" />
        </el-form-item>
  ...
  methods: {
    ...
    generateParentTree(data) {
      ...
      if (!data) {
        ...
        this.editParentTreeData = this.treeData.filter((element, index, array) => {
          return !element.isSystem
        })
      } else {
        this.editParentTreeData = _.cloneDeep(this.treeData)
        // 编辑或移动时，过滤系统分组和自身树(需递归)。系统分组只在第一级
        this.editParentTreeData = this.editParentTreeData.filter((element, index, array) => {
          return !element.isSystem
        })
    ...
    renderContent(h, {
      ...
      return ( <
          ...
          span style = "font-size: 12px;" > {
            data.availableRoles ? data.availableRoles.map(m => m.name).join(' ') : ''
          } < /span> <
          /span> <
